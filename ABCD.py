...
def modinv(a, m): #Modular Inverse
    ...
    if g != 1:
        raise ValueError
    return x % m
class AESCipher: #AES256 Symmetric Encryption
...
E435DE236D525F54759B65E372FCD68EF20FA7111F9E4AFF73", 16)
    ...
    #from SRP: http://www.ietf.org/rfc/rfc5054.txt
...
B40D273651AD6060C13D97FD69AA13C5611A51B9085", 16)
    #2048-bit g, recommended to use by NIST
    qbin="{0:b}".format(q-1) #binary form of q for later use
    ...
    for i in range(0,n):
        ...
        #Each participant requires knowledge of neighbours keys
        ...
        invZ.insert(i,modinv(expZ[i][0],p)) 
    ...
    for i in range(0,n):
        ...
        #Auth_i=Auth(S||{z_j^*,X_j}_j||K_i||i)
    #print(expZ1)
    #print(expX)
    #print(prodK)
    #print(K)
    ...
    for i in range(0,n):
        for j in range(0,n):
            ...
            if j != i:
                ...
                #Auth_j =^? Auth(S||{z_i^*,X_i}_i||K_j||K||j)
    ...
    for i in range(0,n):
        ...
        #sk_i=G(S||{z_j^*,X_j,Auth_j}_j||K_i)
    #print(SK)
    #quick check#
    #for i in range(0,n):
    #    if SK[i] == SK[(i+1)%n]:
