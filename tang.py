...
E435DE236D525F54759B65E372FCD68EF20FA7111F9E4AFF73", 16)
    ...
    #first two concatenated values within the definition of g
    #print(concat1)
    ...
    for i in range(0,n): #A_(i,i-1) & A_(i,i+1)
        ...
        #Then need to make this an integer rather than a hex
        A2c.insert(i,int(a2c[i].hexdigest(),16))
        #A1c_i and A2c_i are broadcasted at the end of the round
    ...
    for i in range(0,n):
        for j in range(0,n):
            if j == i:
                ...
                A2v[i].insert(j,0)
                continue
            #checking values for all j not equal to i
            #(no need to check own value)
            if j != i:
                #Note that the hard work has been done in computation
                #in round two
...
####JAVA IS 25X FASTER THAN PYTHON
#COMPARISON WITH JPAKE+ - AT 20 PARTICIPANTS, JPAKE+ TAKES ~ 1500MS=1.5S
#IF THIS WAS IMPLEMENTED IN PYTHON, ~37.5S
#COMPARISON WITH SPEKE+ - AT 20 PARTICIPANTS, SPEKE+ TAKES ~ 3500MS=3.5S
#IF THIS WAS IMPLEMENTED IN PYTHON, ~87.5S
#THIS PROTOCOL IN PYTHON TAKSE 26S FOR 20 PARTICIPANTS, MUCH FASTER
